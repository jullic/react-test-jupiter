	...
	return (
		<div className={`filter-mobile${isOpenFilter ? ' open' : ''}`}>
			<div className="filter-mobile__header" onClick={openHandler}>
				<FilterItem className='filter-mobile__item-active' isActive={true}>{filters.find(filter => filter.filterTag === activeFilter)?.filterName}</FilterItem>
			...
			<div className="filter-mobile__body">
				{filters.map((filter, index) => <FilterItem key={index} className='filter-mobile__item' isActive={false} onClick={() => activeFilter !== filter.filterTag && changeFilterHandler(filter.filterTag)}>{filter.filterName}</FilterItem>)}
			</div>
		</div>
	);
